...
_Last updated: Phase 7 complete (Tasks 7.1–7.5) — OutboxEvent table + typed event contracts, transactional outbox write in `completeLesson` (XP moved from inline increment to an async subscriber), `FOR UPDATE SKIP LOCKED` relay worker, idempotent `LessonCompleted` XP subscriber, ADR-0008 + Phase 7 course module. **Verification caveat:** the sandbox shell hit an unrecoverable disk-space fault mid-session, so this phase's code was reviewed carefully but not machine-typechecked/runtime-probed the way Phases 0–6 were — verify on host with `pnpm typecheck`, `pnpm db:migrate`, and `pnpm typecheck` in `devmentor` before treating Phase 7 as fully proven. Next: Phase 8 — Notifications (multi-channel, realtime-ready)._
